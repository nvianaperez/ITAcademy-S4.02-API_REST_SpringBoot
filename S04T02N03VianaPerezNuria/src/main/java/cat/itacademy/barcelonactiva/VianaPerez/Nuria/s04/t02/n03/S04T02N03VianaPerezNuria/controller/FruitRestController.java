package cat.itacademy.barcelonactiva.VianaPerez.Nuria.s04.t02.n03.S04T02N03VianaPerezNuria.controller;
...
import cat.itacademy.barcelonactiva.VianaPerez.Nuria.s04.t02.n03.S04T02N03VianaPerezNuria.service.FruitService;
...
@RestController
@CrossOrigin(origins = "http://localhost:8083") //Política de seguridad CORS --> los navegadores por defecto bloquearán las solicitudes realizadas desde un dominio diferente al dominio del servidor que sirve la aplicación
...
public class FruitRestController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Fruit> updateOneFruit(@PathVariable("id") ObjectId id, @RequestBody Fruit fruit) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Fruit> deleteOneFruit(@PathVariable("id") ObjectId id) {
        ResponseEntity<Fruit> fruitResponse = fruitService.checkIdExist(id);
